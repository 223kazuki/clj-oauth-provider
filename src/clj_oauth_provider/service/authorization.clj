(ns clj-oauth-provider.service.authorization
     ...
     [:div.row.main
      [:div.panel-heading
       [:div.panel-title.text-center
        [:h1.title "OAuth 2.0 Provider"]]]
      [:div.main-login.main-center
       [:form.form-horizontal {:method "post" }
        (when-let [error (:error context)]
          [:div.alert.alert-danger error])
        [:div.form-group
         [:label.cols-sm-2.control-label {:for "username"} "Your ID"]
         [:div.cols-sm-10
          [:div.input-group
           [:span.input-group-addon [:i.fa.fa-user.fa {:aria-hidden true}]]
           [:input.form-control {:type "text" :name "username" :id "name" :placeholder "Enter your ID"}]]]]
        [:div.form-group
...
(defn new-code [component client]
  (let [{:keys [client_id redirect_uri]} client
        code (re-rand #"[a-zA-Z0-9]{10}")]
    (swap! (:code-cache component) assoc code {:client client :used? false})
    code))

(defmethod ig/init-key :clj-oauth-provider.service/authorization [_ options]
  (assoc options
         ;; Authorization Code: 10 min.
         :code-cache (atom (cache/ttl-cache-factory {} :ttl (* 10 60 1000)))
         ;; Access Token: 30 min.
...
(defmethod ig/halt-key! :clj-oauth-provider.service/authorization [_ options]
  (dissoc options :code-cache :token-cache :refresh-token-cache))
